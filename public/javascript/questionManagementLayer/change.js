...
function newMult($root, data) {
    ...
    for (let i = 0; i < data.options.length; ++i) {
        ...
        $dom.find('.ac').append($(`
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="checkbox" value="${i + 1}" name="op">
    ...
    for (let i = 0; i < answer.length; ++i) {
        $dom.find('input.form-check-input').eq(answer[i] - 1).prop('checked', true);
    }
...
function newJug($root, data) {
    ...
    data.answer ? $dom.find('.answer option:eq(1)').prop('selected', true) : $dom.find('.answer option:eq(0)').prop('selected', true)
...
function newCom($root, data) {
    ...
    for (let i = 0; i < data.smallQuestions.length; ++i) {
        let $smcc = $(`
            <div id="sm${i + 1}" class="smcc m-3 rounded" data-type="${data.smallQuestions[i].type}">
                <div class="card">
                    <div class="card-header">
                        <button class="btn btn-link c" data-toggle="collapse" data-target="#cl${i + 1}">
                            小问${i + 1}
                        </button>
                        <button class="btn btn-link float-right text-danger">删除</button>
                    </div>
                    <div id="cl${i + 1}" class="collapse" data-parent="#sm${i + 1}">
                        <div class="card-body">
                        </div>
                    </div>
                </div>
        ...
        render(data.smallQuestions[i].type, $smcc.find('.card-body'), data.smallQuestions[i]);
    ...
    $dom.find('#addsm').click(function () {
        layer.open({
            ...
            yes: function (index) {
                ...
                $('#smc').append($smcc);
                renderN($('#qtype').val(), $smcc.find('.card-body'));
            },
            no: function (index) {
                layer.close(index);
            }
